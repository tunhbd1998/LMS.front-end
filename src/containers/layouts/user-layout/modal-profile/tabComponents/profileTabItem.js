...
const ProfileTabItem = ({profile, editMode,bindProfile}) => {
    ...
    <Container style={{padding:0 }}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={3}>
            <Dropdown label="Giới tính" 
            {... get(bindProfile, ['bindGender'] || 'null' )}
            defaultValue={profile.gender === 1 ? 'male' : 'female' } data={gender} disabled = {!editMode}/>
          </Grid>
          <Grid item xs={6}>
            <Input label="Số CMND"
          ...
          <Grid item xs={6}>
            ...
            defaultValue={get(profile,['birthday'] || 'null')} disabled = {!editMode}/>
          </Grid>
          <Grid item xs={12}>
            <Input label="Số điện thoại" 
            {... get(bindProfile, ['bindPhone'] || 'null' )}
            defaultValue ={get(profile,['phone'] || 'null')} disabled = {!editMode}/>
          </Grid>
          <Grid item xs={12}>
            <Input
              label="Địa chỉ email"
